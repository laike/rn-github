...
const componentName = ({data, ...rest}) => {
  ...
  return Platform.OS === 'android' ? (
    <TouchableNativeFeedback>
      <View style={styles.item}>
        ...
        <View style={styles.des}>
          {data.type === 'IssuesEvent' || data.type === 'IssueCommentEvent' ? (
            <Text>{data.payload.issue.title}</Text>
          ) : (
            <View />
          )}
          ...
          {data.type === 'PushEvent' ? (
            data.payload.commits.map((commit, index) => (
              <Text key={index}>{commit.message}</Text>
            ))
          ) : (
            <View />
          )}
    ...
    <TouchableHighlight>
      <View style={styles.item}>
        <View style={styles.top}>{renderTop()}</View>
        <View style={styles.left}>
